...
- **PowerShell-мониторинг:** подписка на Windows Event Log 4104 (Script Block Logging) для обнаружения вредоносных PS-скриптов в реальном времени (из скила `analyzing-powershell-script-block-logging`)
- **Ransomware-паттерны:** обнаружение массовых переименований файлов (>50 файлов/сек с сменой расширения) как признак шифровальщика (из скила `analyzing-ransomware-network-indicators`)

### 4.3 Карантин (QuarantineManager)

- Изолированный файл шифруется **AES-256-CBC с уникальным ключом на файл** (CSPRNG), ключ обёрнут RSA — тот же стандарт что у LockBit/REvil, но мы его используем для защиты, а не атаки
- Файл переносится в `%AppData%\AppName\Quarantine\`
- В SQLite (`quarantine.db`): оригинальный путь, имя угрозы, дата, SHA256, **confidence score** (0–100)
- Действия: **Восстановить** (расшифровать на исходный путь) / **Удалить навсегда**
- GUI: таблица с колонками Имя файла | Угроза | Confidence | Дата | Действия
...
- **Расширенная оценка риска** (из скилов `analyzing-malware-persistence-with-autoruns` и `analyzing-windows-amcache-artifacts`):

| Признак | +Риск |
|---|---|
| Нет Authenticode-подписи | +30 |
| Запущен из `%TEMP%`, `%APPDATA%` | +25 |
| Активные сетевые соединения | +20 |
| LoLBins (PowerShell, WScript, CertUtil с нетипичными аргументами) | +35 |
| Запущен из папки загрузок | +15 |
| Совпадает с записью в Prefetch/AmCache но путь изменился | +40 |

- Цветовая индикация: зелёный (0–29) / жёлтый (30–59) / красный (60+)
- GUI: таблица процессов + кнопка завершить / поместить в карантин

### 4.5 Оптимизация системы (SystemOptimizer)

**Вкладка «Автозагрузка»:**
- **18+ категорий ASEP** (из скила `analyzing-malware-persistence-with-autoruns`, MITRE T1547/T1543/T1053/T1574):
  - Реестр: `HKCU/HKLM\...\Run`, `RunOnce`, `RunServices`
  - Scheduled Tasks (через Task Scheduler API)
  - Службы Windows (тип StartType = Automatic)
  - Папки Startup (`%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup`)
  - Browser extensions (список установленных)
  - Boot drivers (`HKLM\SYSTEM\CurrentControlSet\Services` с Start=0/1)
- Действия: отключить / включить / удалить
- Каждый элемент проверяется через `ScanEngine` — подозрительные выделяются красным
...
### 4.6 Оценка угрозы (ThreatScorer)

Каждая найденная угроза получает **confidence score 0–100** (из скила `analyzing-indicators-of-compromise`):

| Уровень | Score | Действие |
|---|---|---|
| Block | ≥70 | Автоматический карантин |
| Monitor | 40–69 | Предупреждение пользователю, решение за ним |
| Whitelist | <40 | Логируется, не блокируется |

Факторы score: число срабатываний ClamAV-детекторов, совпадение YARA-правил, совпадение в MalSearcher, поведенческие признаки, путь файла.

**IOC TTL-политика:** хеши в локальной базе помечаются датой добавления. Записи старше 90 дней без повторных совпадений — помечаются как «требуют обновления».

### 4.7 Обновление баз (SignatureUpdater)

- **ClamAV:** скачать `main.cvd`, `daily.cvd`, `bytecode.cvd` с официальных зеркал
- **YARA:** обновить набор правил из GitHub (Yara-Rules/rules) — Sigma-совместимые правила версионируются в Git
